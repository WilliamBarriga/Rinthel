 ...
 * by Rinthel-general (whisper.cpp for STT, a Piper wrapper for TTS). Both
 * require a bearer token now — generated by Rinthel-general's installer into
 * services/whisper-stt/.env and services/tts-piper/.env, copied here as
 * WHISPER_AUTH_TOKEN/TTS_AUTH_TOKEN. The token never reaches the browser:
 * the browser only ever talks to this same-origin /api/voice/* route.
 */
...
const WHISPER_AUTH_TOKEN = process.env.WHISPER_AUTH_TOKEN || "";
    ...
    async (req, res) => {
      ...
      try {
        const upstream = await fetch(`${WHISPER_BASE_URL}/inference`, {
          method: "POST",
          headers: { Authorization: `Bearer ${WHISPER_AUTH_TOKEN}` },
          body: form,
        });
        if (!upstream.ok) {
